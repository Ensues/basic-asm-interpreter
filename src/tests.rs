...
pub fn simple_test() {
    ...
    for (prg, exp) in simple_programs.iter().zip(expected) {
        let actual = interpreter::AssemblerInterpreter::interpret(*prg);
...
pub fn test_tokenizer() {
    ...
    for prg in simple_programs.iter() {
        let potential = interpreter::tokenize(*prg);
        if let Some(actual) = potential {
            for l in actual.iter() {
                println!("{}", l);
            }
            println!("|")
        }
